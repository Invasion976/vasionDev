...
Citizen.CreateThread(function()
	...
	while true do
		timer = 3000
		for k,mark in pairs(Config.companies) do
			local x,y,z = table.unpack(mark.coordinate)
...
AddEventHandler('truck_logistics:open', function(data,update)
	-- Calculate the distance and reward for each contract
	local x1,y1,z1 = table.unpack(GetEntityCoords(PlayerPedId()))
	for k,v in pairs(data['trucker_available_contracts']) do
		...
		-- if distance > 50 then
		local firstVect = vector3(x1,y1,z1)
		local seconVect = vector3(x2,y2,z2)
		
		
		distance = #(firstVect.xy - seconVect.xy)
		-- end
		--data['trucker_available_contracts'][k]['distance'] = tonumber(string.format("%.2f", (distance/1000)))
		data['trucker_available_contracts'][k]['distance'] = tonumber(string.format("%.2f", distance/1609))
		data['trucker_available_contracts'][k]['reward'] = tonumber(string.format("%.f", (v.distance * v.price_per_km)))
	end
...
RegisterNUICallback('startJob', function(data, cb)
	if cooldown == nil then
		...
		if not loading then
			...
			TriggerEvent("Notify","important",Lang[Config.lang]['loading_trailer'])
			closeUI()
		end
...
RegisterNUICallback('repairTruck', function(data, cb)
	if truck and not rentTruck then
		TriggerEvent("Notify","denied",Lang[Config.lang]['store_truck'])
		return
	end
...
RegisterNUICallback('sellTruck', function(data, cb)
	if cooldown == nil then
		...
		if truck and not rentTruck then
			TriggerEvent("Notify","denied",Lang[Config.lang]['store_truck_2'])
			return
		end
...
RegisterNUICallback('spawnTruck', function(data, cb)
	if cooldown == nil then
		...
		if not loading then
			...
			TriggerEvent("Notify","important",Lang[Config.lang]['loading_truck'])
			closeUI()
		end
...
AddEventHandler('truck_logistics:spawnTruck', function(truck_data)
	...
	if truck then
		TriggerEvent("Notify","denied",Lang[Config.lang]['already_has_truck'])
		return
	end
	
	loading = true
	local garage = Config.companies[currentCompany]['garage_coordinate']
	local i = #garage
	local x,y,z,h
	while i > 0 do
		x,y,z,h = table.unpack(garage[i])
		local checkPos = IsSpawnPointClear({['x']=x,['y']=y,['z']=z},5.001)
		if checkPos == false then
			if i <= 1 then
				TriggerEvent("Notify","denied",Lang[Config.lang]['occupied_places'])
				loading = false
				return
			end
		else
	...
	TriggerEvent("Notify","success",Lang[Config.lang]['already_is_in_garage'])
	loading = false
	...
	while IsEntityAVehicle(truck) do
		...
		if veh == truck then
			...
			for k,v in pairs(Config.companies) do
				for k,mark in pairs(v.garage_coordinate) do
...
AddEventHandler('truck_logistics:startContract', function(data,job_distance,reward,truck_data)
	...
	if trailer or rentTruck then
		TriggerEvent("Notify","denied",Lang[Config.lang]['already_has_cargo'])
		return
	end
	...
	if truck and data.contract_type == 0 then
		TriggerEvent("Notify","denied",Lang[Config.lang]['must_store_truck'])
		return
	end
	...
	if data.contract_type == 0 then
		local garage = Config.companies[key]['garage_coordinate']
		local i = #garage
		while i > 0 do
			x,y,z,h = table.unpack(garage[i])
			local checkPos = IsSpawnPointClear({['x']=x,['y']=y,['z']=z},5.001)
	...
	local loads = Config.companies[key]['load_coordinate']
	i = #loads
	while i > 0 do
		x,y,z,h = table.unpack(loads[i])
		local checkPos = IsSpawnPointClear({['x']=x,['y']=y,['z']=z},5.001)
		if checkPos == false then
			if i <= 1 then
				if rentTruck then
	...
	TriggerEvent("Notify","success",Lang[Config.lang]['started_job'])
	loading = false
	...
	while IsEntityAVehicle(trailer) do 
		...
		local ped = PlayerPedId()
		veh = GetVehiclePedIsIn(ped,false)
		local firstVect = (GetEntityCoords(ped))
		local seconVect = (vector3(x,y,z))
		
		
		local distance = #(firstVect.xy - seconVect.xy)
		...
		if bodyhealth <= 150 then
			PlaySoundFrontend(-1, "PROPERTY_PURCHASE", "HUD_AWARDS", 0)
			TriggerEvent("Notify","denied",Lang[Config.lang]['failed'])
		...
		if IsControlPressed(0,Config.contracts['cancel_contract']) then
			DeleteVehicle(trailer)
			RemoveBlip(trailer_blip)
			RemoveBlip(route_blip)
			trailer = nil
	...
	if IsEntityAVehicle(truck) and data.contract_type == 0 then
            -- {1246.07,-3142.42,5.63,270.00}
    		route_blip = AddBlipForCoord(1246.07,-3142.42,270.00)
            SetBlipSprite(route_blip,1)
            SetBlipColour(route_blip,5)
            SetBlipAsShortRange(route_blip,false)
            BeginTextCommandSetBlipName("STRING")
...
function resetLoading()
	...
	if loading == true then
		TriggerEvent("Notify","denied",Lang[Config.lang]['loading_fail'])
		loading = false
	end
end
...
AddEventHandler('truck_logistics:vehicleClientLock', function()
	...
	if DoesEntityExist(v) and IsEntityAVehicle(v) then
		...
		if lock == 1 then
			...
			TriggerEvent("Notify","important",Lang[Config.lang]['locked'],8000)
		else
			SetVehicleDoorsLocked(v,1)
			TriggerEvent("Notify","important",Lang[Config.lang]['unlocked'],8000)
		end
    ...
    while true do
        ...
        for k,v in pairs(node) do
            if (cache[node] == nil) or (cur_index >= cache[node]) then
                ...
                -- This is necessary for working with HUGE tables otherwise we run out of memory using contract on huge strings
