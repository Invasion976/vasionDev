window.addEventListener('message', function (event) {
	...
	if (item.showmenu){
		...
		var contracts = item.data.trucker_available_contracts;
		var users = item.data.trucker_users;
		var myTrucks = item.data.trucker_trucks;
		var drivers = item.data.trucker_drivers;
		var loans = item.data.trucker_loans;
		if(item.update != true){
		...
		$('#new-contracts-1').append('New contracts every ' + (config.cooldown*2) + ' min');
		$('#new-contracts-2').empty();
		$('#new-contracts-2').append('New contracts every ' + (config.cooldown*2) + ' min');
		
		$('#profile-money').empty();
		$('#profile-money').append(new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money, maximumFractionDigits: 0, minimumFractionDigits: 0 }).format(users.money));
		$('#bank-money').empty();
		$('#bank-money').append(new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money, maximumFractionDigits: 0, minimumFractionDigits: 0 }).format(users.money));
		
		$('#profile-money-earned').empty();
		$('#profile-money-earned').append(new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money, maximumFractionDigits: 0, minimumFractionDigits: 0 }).format(users.total_earned));
		...
		$('#profile-exp-2').empty();
		var exp_r = 0
		if (users.exp >= config.exp_per_level[config.exp_per_level.length-1]){
			exp_r = 100;
		}else if (config.player_level == 0) {
			max = config.exp_per_level[config.player_level]
			exp = users.exp
			exp_r = Math.round((exp*100)/max);
		}else{
			for (const key in config.exp_per_level) {
				if(users.exp < config.exp_per_level[key]){
					max = config.exp_per_level[key] - config.exp_per_level[key-1]
					exp = users.exp - config.exp_per_level[key-1]
					exp_r = Math.round((exp*100)/max);
					if(exp_r >= 0){
						break;
					}
		...
		$('#profile-distance-traveled').append(users.traveled_distance.toFixed(2) + 'mi');
					...
					<div class="d-flex flex-row align-items-center"><i class="fa fa-check-circle checkicon"></i>
						<div class="ml-2">
							...
							<div class="d-flex flex-row mt-1 text-black-50 date-time">
								...
								<div class="ml-3"><i class="fas fa-coins"></i><span class="ml-2">Reward: ` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money }).format(contract.reward) + `</span></div>
							</div>
						</div>
					</div>
					...
					<div class="card-body pt-0 px-0">
						...
						<div class="d-flex flex-row justify-content-between px-3 pb-4">
							...
							<div class="d-flex flex-column">
								<h5 class="mb-0">` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money }).format(truck.price) + `</h5>
							</div>
						</div>
		...
		for (const truck of myTrucks) {
			...
			if(truck.driver == 0){
				$('#truck-name').append('('+config.dealership[truck.truck_name].name+')');
				var color = "";
				...
				$('#diagnostic-body').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.body + ` %</h3><span>Fix Body (` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money }).format((100-truck.body)*config.repair_value.body) + `)</span>
						</div>
				...
				$('#diagnostic-engine').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.engine + ` %</h3><span>Fix Engine (` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money }).format((100-truck.engine)*config.repair_value.engine) + `)</span>
						</div>
						<div class="align-self-center">
							<i class="fas fa-tools ` + color + ` font-large-2 float-right"></i>
						</div>
				...
				$('#diagnostic-transmission').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.transmission + ` %</h3><span>Fix Transmission (` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money }).format((100-truck.transmission)*config.repair_value.transmission) + `)</span>
						</div>
						<div class="align-self-center">
							<i class="fas fa-tools ` + color + ` font-large-2 float-right"></i>
						</div>
				...
				$('#diagnostic-wheels').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.wheels + ` %</h3><span>Fix Wheels (` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money }).format((100-truck.wheels)*config.repair_value.wheels) + `)</span>
						</div>
						<div class="align-self-center">
							<i class="fas fa-tools ` + color + ` font-large-2 float-right"></i>
						</div>
				...
				list_item += `
					<div class="card user-card">
						<div class="card-block">
							...
							<p class="text-muted">Price: ` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money }).format(driver.price) + ` + ` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money }).format(driver.price_per_km) + `/km</p>
							<hr>
					...
					<li class="d-flex justify-content-between">
						<div class="d-flex flex-row align-items-center">
							...
							<div class="ml-2">
								...
								<div class="d-flex flex-row mt-1 text-black-50 date-time">
									<div>
										<i class="fas fa-coins"></i><span class="ml-2">Price: ` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money }).format(driver.price) + ` + ` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money }).format(driver.price_per_km) + `/km</span>
									</div>
									<div class="ml-3">
		...
		$('#loan-title').empty();
		$('#loan-title').append('Get loans to invest in your company (Maximum active loans: ' + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money }).format(config.max_loan) + ')');
		
		$('#loan-1').empty();
		list_item = '<h3 class="success darken-1">' + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money }).format(config.loans[0][0]) + '</h3><span>pay ' + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money }).format(config.loans[0][1]) + ' at day</span>';
		$('#loan-1').append(list_item);
		$('#loan-2').empty();
		list_item = '<h3 class="success darken-1">' + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money }).format(config.loans[1][0]) + '</h3><span>pay ' + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money }).format(config.loans[1][1]) + ' at day</span>';
		$('#loan-2').append(list_item);
		$('#loan-3').empty();
		list_item = '<h3 class="success darken-1">' + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money }).format(config.loans[2][0]) + '</h3><span>pay ' + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money }).format(config.loans[2][1]) + ' at day</span>';
		$('#loan-3').append(list_item);
		$('#loan-4').empty();
		list_item = '<h3 class="success darken-1">' + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money }).format(config.loans[3][0]) + '</h3><span>pay ' + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money }).format(config.loans[3][1]) + ' at day</span>';
		...
		for (const loan of loans) {
			list_item = `
				<li class="d-flex justify-content-between">
					<div class="d-flex flex-row align-items-center"><i class="fa fa-check-circle checkicon"></i>
						<div class="ml-2">
							<h6 class="mb-0">` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money }).format(loan.loan) + `</h6>
							<div class="d-flex flex-row mt-1 text-black-50 date-time">
								<div><i class="fas fa-coins"></i></i><span class="ml-2">Remaining: ` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money }).format(loan.remaining_amount) + `</span></div>
								<div class="ml-3"><i class="fas fa-coins"></i></i><span class="ml-2">Daily cost: ` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.money }).format(loan.day_cost) + `</span></div>
							</div>
						</div>
					</div>
					<div class="d-flex flex-row align-items-center">
...
function post(name,data){
    ...
    console.log("http://RRP-Trucking/" + name, JSON.stringify(data))
	$.post("http://RRP-Trucking/" + name, JSON.stringify(data),function(datab){
		if (datab != "ok"){
			console.log(datab);
		}
	});
